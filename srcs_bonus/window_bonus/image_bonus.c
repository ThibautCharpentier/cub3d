...
void	init_image(t_win *win)
{
	...
	win->door.img = image(&win->door, win->mlx, "./textures/DOOR_1C.xpm");
	if (!win->no.img || !win->so.img || !win->we.img
		|| !win->ea.img || !win->door.img)
	{
	...
	win->door.addr = (int *) mlx_get_data_addr(win->door.img,
			&win->door.bits_per_pixel, &win->door.line_length,
			&win->door.endian);
}
...
void	destroy_img(t_win *win)
{
	...
	mlx_destroy_image(win->mlx, win->door.img);
}
